...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6, weight=2):
    assert mode in ['iou', 'iof', 'giou', 'wd', 'kl','center_distance2','exp_kl','kl_10', 'gjsd',
                    'kfiou', 'kfiou2', 'kfiou3', 'kfiou4', 'kfiounwd', 'BGSM'], f'Unsupported mode {mode}'
    ...
    if mode == 'kfiou':
        ...
        # kld_pt
        term1 = ((center2 - center1)[:, :, 0] ** 2) / (w1 ** 2) + ((center2 - center1)[:, :, 1] ** 2) / (h1 ** 2)

        # kld_tp     
        # term1 = ((center2 - center1)[:,:,0] ** 2) / (w2 ** 2) + ((center2 - center1)[:,:,1] ** 2) / (h2 ** 2)

        return KFIoU + 1 / (2 + term1)
    ...
    if mode == 'kfiounwd':
        ...
        center_distance = (center2 - center1)[:, :, 0] ** 2 + (center2 - center1)[:, :, 1] ** 2 + eps
        wassersteins = torch.sqrt(center_distance + wh_distance)
        normalized_wasserstein = torch.exp(-wassersteins / 12.7)

        return KFIoU + normalized_wasserstein
    ...
    if mode == 'kfiou3':
        ...
        ones = torch.ones_like(KFIoU)
        KFIoU2 = torch.where(KFIoU > 0.1, ones, KFIoU)
        return KFIoU2 * normalized_wasserstein
    ...
    if mode == 'kfiou4':
        ...
        return KFIoU + 1 / (1 + torch.log(torch.sqrt((w1 - w2) ** 2 + (h1 - h2) ** 2 + center_distance) + 1))
