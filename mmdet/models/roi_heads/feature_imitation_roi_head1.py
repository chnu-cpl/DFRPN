...
class FIRoIHead1(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(FIRoIHead1, self).__init__(
        ...
        self.attention = MultiHeadSelfAttention(256, 8)
        self.flatten_embed = nn.Linear(256 * 7 * 7, 256)
    ...
    #         comp_convs.append(
    #             DeformableConvModule(in_channels, in_channels, 3, norm_cfg=norm_cfg, act_cfg=act_cfg)
    #         )
    #     return comp_convs
    ...
    def _add_comp_convs(self, in_channels, roi_feat_size, norm_cfg, act_cfg):
        ...
        # for i in range(1):
        #     comp_convs.append(
        #         ConvModule(in_channels, in_channels, 3, norm_cfg=norm_cfg, act_cfg=act_cfg, dilation=2, groups=256),
        #     )
        # for i in range(1):
        #     comp_convs.append(
        #         ConvModule(in_channels, in_channels, 3, norm_cfg=norm_cfg, groups=1),
        #     )
        # for i in range(1):
        #     comp_convs.append(
    ...
    def _bbox_forward(self, x, rois):
        ...
        feat_weights = self.attention(y)
        # attn_output = F.adaptive_max_pool2d(attn_output, (1, 1))
        # feat_weights = attn_output1
        # attn_output = F.adaptive_avg_pool2d(attn_output1, (1, 1))
        # import pdb
        # pdb.set_trace()
        for conv in self.comp_convs:
            feat_weights = conv(feat_weights)
        # feat_weights = feat_weights + attn_output
    ...
    def _bbox_forward_train(self, x, assign_results, sampling_results,
        ...
        for i in range(len(sampling_results)):
            ...
            # import pdb
            # pdb.set_trace()
            # b, c, h, w = hq_feats.size()
            # if b > 0:
            #     # flatten = self.Flatten(feat_weights)
            #     x = hq_feats.view(b, -1, 256)  # (b, 49, 256)
            #     attn_output1 = self.attention(x, x, x, mask=None)
            #     attn_output2 = attn_output1.transpose(1, 2).view(b, c, h, w)
            #     # feat_weights = attn_output2.clone()
            #     # attn_output3 = self.Flatten(attn_output2)
            #     # attn_output4 = self.flatten_embed(attn_output3)
            #     attn_output_hq = F.adaptive_avg_pool2d(attn_output2, (1, 1))
            # b, c, h, w = hq_feats.size()
            # if b > 0:
            #     w = hq_feats
            #     w = F.adaptive_avg_pool2d(w, (1, 1))
            #     hq_feats = self.attention(w)
                # attn_output_hq = F.adaptive_avg_pool2d(attn_output2, (1, 1))
            #     attn_output2 = attn_output1.transpose(1, 2)
            #     attn_output3 = self.Flatten(attn_output2)
            #     attn_output4 = self.flatten_embed(attn_output3)
            #     attn_output = attn_output4.view(-1, 256, 1, 1)
            with torch.no_grad():
                for conv in self.comp_convs:
                    hq_feats = conv(hq_feats)  # [num_proposals, 256, 1, 1]
            # if b > 0:
            #     hq_feats = hq_feats + attn_output_hq
            ...
            if len(hq_inds) == 0:    # no high-quality gt in current image
                ...
                #
                # b, c, h, w = aug_hq_gt_roi_feats.size()
                # if b > 0:
                #     z = aug_hq_gt_roi_feats
                #     z = F.adaptive_avg_pool2d(z, (1, 1))
                #     aug_hq_gt_roi_feats = self.attention(z)
                #     attn_output2 = attn_output1.transpose(1, 2).view(b, c, h, w)
                #     # feat_weights = attn_output2.clone()
                #     # attn_output3 = self.Flatten(attn_output2)
                #     # attn_output4 = self.flatten_embed(attn_output3)
                #     attn_output_aug = F.adaptive_avg_pool2d(attn_aug_hq_gt_roi_feats, (1, 1))
                with torch.no_grad():
                    for conv in self.comp_convs:
                        aug_hq_gt_roi_feats = conv(aug_hq_gt_roi_feats)
                # if b > 0:
                #     aug_hq_gt_roi_feats = aug_hq_gt_roi_feats + attn_output_aug
    ...
    def _aug_hq_gt_bboxes(self, hq_gt_bboxes, img_w):
        with torch.no_grad():
            ...
            # gt box translation
            trans_candi = gt_bboxes.repeat(1, 4 * trans_num, 1)
            w = hq_gt_bboxes[:, 3] - hq_gt_bboxes[:, 1]
            h = hq_gt_bboxes[:, 2] - hq_gt_bboxes[:, 0]
            wh = torch.cat([w.view(-1, 1), h.view(-1, 1)], dim=1).unsqueeze(1)
            inter_mat = torch.cat(
                [torch.eye(2), torch.eye(2) * (-1)], dim=0).cuda()
            wh_mat = wh * inter_mat
            scaled_wh = torch.cat(
                [r * wh_mat for r in trans_ratios], dim=1)
            trans_wh = scaled_wh.repeat(1, 1, 2)
            trans_gt_bboxes = trans_candi + trans_wh
            trans_gt_bboxes = torch.clamp(trans_gt_bboxes, 0, img_w)

            # gt box rescale
            rescaled_gt_bboxes = self.rescale_gt_bboxes(
                hq_gt_bboxes, rescale_ratios)
            rescaled_gt_bboxes = torch.clamp(rescaled_gt_bboxes, 0, img_w)

            # Randomly apply horizontal flip
            # if random.random() > 0.5:
            #     trans_gt_bboxes[:, :, [1, 3]] = img_w - trans_gt_bboxes[:, :, [3, 1]]
            #     rescaled_gt_bboxes[:, :, [1, 3]] = img_w - rescaled_gt_bboxes[:, :, [3, 1]]

            # Randomly apply copy-paste
            # if random.random() > 0.5:
            copy_paste_idx = random.randint(0, num_gts - 1)
            trans_gt_bboxes[copy_paste_idx] = trans_gt_bboxes[copy_paste_idx].clone()
            rescaled_gt_bboxes[copy_paste_idx] = rescaled_gt_bboxes[copy_paste_idx].clone()
            ...
            for i in range(num_gts):
                aug_gt_bboxes.append(
                    torch.cat([trans_gt_bboxes[i], rescaled_gt_bboxes[i]],
                              dim=0))
            aug_gt_bboxes = torch.cat(aug_gt_bboxes, dim=0)
            aug_num_per_hq_gt = 4 * trans_num + rescale_num
    ...
    #         gt_bboxes = hq_gt_bboxes.unsqueeze(1)
    #         # # 随机反转，随机镜像
    #         flip_mirror_gt_bboxes = []
    #         for i in range(num_gts):
    #             if random.random() < 0.5:
    #                 flipped_bbox = hq_gt_bboxes[i:i+1].clone()
    #                 # 随机翻转
    #                 angle = random.uniform(-45, 45)
    #                 cx = (hq_gt_bboxes[i, 0] + hq_gt_bboxes[i, 2]) * 0.5
    #                 cy = (hq_gt_bboxes[i, 1] + hq_gt_bboxes[i, 3]) * 0.5
    #                 for j in range(4):
    #                     x, y = hq_gt_bboxes[i, j % 2], hq_gt_bboxes[i, (j % 2) + 2]
    #                     x, y = x - cx, y - cy
    ...
    #                     x_new, y_new = x_new + cx, y_new + cy
    #                     flipped_bbox[0, j % 2] = x_new
    #                     flipped_bbox[0, (j % 2) + 2] = y_new
    #                 flip_mirror_gt_bboxes.append(flipped_bbox)
    #             else:
    #                 flip_mirror_gt_bboxes.append(hq_gt_bboxes[i: i+1].clone())
    #         copy_paste_gt_bboxes = torch.cat(flip_mirror_gt_bboxes, dim=0)
    #         trans_candi = copy_paste_gt_bboxes.unsqueeze(1).repeat(1, 4 * trans_num, 1)  # (num_gts, 4*trans_num, 4)
    #
    #         # gt box translation
    #         # trans_candi = gt_bboxes.repeat(1, 4 * trans_num, 1)  # (num_gts, 4*trans_num, 4)
    #         w = hq_gt_bboxes[:, 3] - hq_gt_bboxes[:, 1]
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        N, C, H, W = x.shape
        # Flatten spatial dimensions and combine them with the channel dimension
        x = x.view(N, C, H * W).transpose(1, 2)  # Shape: (N, H*W, C)

        # Split the embedding into self.heads different pieces
        x = x.reshape(N, H * W, self.heads, self.head_dim)
        ...
        out = torch.einsum("nhql,nlhd->nqhd", [attention, values]).reshape(
            N, H * W, self.heads * self.head_dim
        )
...
class DeformableConvModule(ConvModule):
    def __init__(self, *args, **kwargs):
        super(DeformableConvModule, self).__init__(*args, **kwargs)

    def _conv_layer(self, *args, **kwargs):
        if self.conv_cfg is None:
            return super(DeformableConvModule, self)._conv_layer(*args, **kwargs)
        else:
            return self.conv_cfg['type'](*args, **kwargs, deformable_groups=1)